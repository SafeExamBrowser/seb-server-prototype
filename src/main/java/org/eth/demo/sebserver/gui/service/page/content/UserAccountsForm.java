...
@Lazy
@Component
public class UserAccountsForm implements TemplateComposer {

    private final RestServices restServices;
    private final WidgetFactory widgetFactory;

    public UserAccountsForm(final RestServices restServices, final WidgetFactory widgetFactory) {
    ...
    public void compose(final PageContext composerCtx) {
        ...
        final Label labelLocalized = this.widgetFactory.labelLocalized(
                content, "h2", new LocTextKey(
                        "org.sebserver.useraccounts.info.title"));
        labelLocalized.setLayoutData(new GridData(SWT.TOP, SWT.LEFT, true, false));

        this.widgetFactory.labelLocalized(content, "h3", new LocTextKey("org.sebserver.useraccounts.info"));
