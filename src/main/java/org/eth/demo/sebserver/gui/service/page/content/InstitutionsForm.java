...
@Lazy
@Component
public class InstitutionsForm implements TemplateComposer {

    private final RestServices restServices;
    private final WidgetFactory widgetFactory;

    public InstitutionsForm(final RestServices restServices, final WidgetFactory widgetFactory) {
    ...
    public void compose(final PageContext composerCtx) {
        ...
        final Label labelLocalized = this.widgetFactory.labelLocalized(
                content, "h2", new LocTextKey(
                        "org.sebserver.institutions.info.title"));
        labelLocalized.setLayoutData(new GridData(SWT.TOP, SWT.LEFT, true, false));

        this.widgetFactory.labelLocalized(content, "h3", new LocTextKey("org.sebserver.institutions.info"));
